...
let saveDetailInforDoctor = (inputData) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const missingKeyError = checkRequiredKeys(inputData, requiredKeys);
      if (missingKeyError) {
        resolve(missingKeyError);
        return;
      } else {
        ...
        if (inputData.action === "CREATE") {
          ...
          if (doctorMarkdown) {
            ...
            doctorMarkdown.contentMarkdown = inputData.contentMark;
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (
        ...
        //update patient status from S2 to S3
        ...
        resolve({
          errCode: 0,
          message: "confirm schedule and send email success"
        });
      }
    } catch (error) {
      reject(error);
